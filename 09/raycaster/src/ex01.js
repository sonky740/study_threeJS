...
export default function example() {
  ...
  const boxMaterial = new THREE.MeshPhongMaterial({ color: 'plum' });
  const boxMesh = new THREE.Mesh(boxGeometry, boxMaterial);
  boxMesh.name = 'box';

  const torusGeometry = new THREE.TorusGeometry(2, 0.5, 16, 100);
  const torusMaterial = new THREE.MeshPhongMaterial({ color: 'lime' });
  ...
  function draw() {
    ...
    const direction = new THREE.Vector3(0, 0, -100); // 정규화된 벡터 => 아니면 normalize()
    direction.normalize();
